function AS_ShowGCmatrix_residual(Ptype)
GC_RES.Ptype = Ptype;
...
GC_RES.fig = figure('position',[MIDPOINT(1)-POSSIZE(1)/2,MIDPOINT(2)-POSSIZE(2)/2,POSSIZE(1)/1,POSSIZE(2)/1],'name','GC Matrix(Using residual)');
...
for i = 1:size(listbox,1)
    listboxthr{i,1} = [ListVarName{i,1},',max:',num2str(maxV(i))];
end
...
for i = 1:size(strings,1)
    strtemp = strings{i};
    K = find(strtemp==':');
    maxv = strtemp(K+1:end);
    MAXV = str2num(maxv);
    K1 = find(strtemp==',');
    varname = strtemp(1:K1-1);
    Answtemp = inputdlg(['threshold setup: ',varname,',max:',maxv],['threshold setup: ',varname,',max:',maxv],1,{num2str(MAXV/2)});
    LISTSTRING{i,1} = [varname,',Threshold,[',Answtemp{1},'],max:',maxv];
end
...
if GC_RES.Ptype==1||(GC_RES.Ptype~=0&&get(GC_RES.TheType1,'val')==1) % With No Pval    
    strings = get(GC_RES.TheType1_changelist,'string');
    for i = 1:size(strings,1)
        strtemp = strings{i};
        Ktemp1 = find(strtemp=='[');
        ...
        if isempty(Ktemp1)
            ...
            thr(i) = str2num(strtemp(Ktemp1+1:Ktemp-1));
        end
        MATSHOW = MatrixShow{i,1};
        MATSHOW(MATSHOW<thr(i))=0;
...
for i = 1:size(MATSHOWUse,1)
    ...
    imagesc(MATSHOWUsed_S);title(ListVarName{i,1});
    hold on;
    load('COL_Residual.mat');
    colormap(COL_Residual);
    for j = 1:nsize(2)
