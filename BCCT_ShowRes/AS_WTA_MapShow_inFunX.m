function AS_WTA_MapShow_inFunX(inmixdir,outdir,extendfactor1)
...
%%
minx = min(rangx)-(max(rangx)-min(rangx))*extendfactor1;
maxx = max(rangx)+(max(rangx)-min(rangx))*extendfactor1;
miny = min(rangy)-(max(rangy)-min(rangy))*extendfactor2;
maxy = max(rangy)+(max(rangy)-min(rangy))*extendfactor2;
minz = min(rangz)-(max(rangz)-min(rangz))*extendfactor3;
maxz = maxs(rangz)+(max(rangz)-min(rangz))*extendfactor3;
% for i = 1:length(rangz)
